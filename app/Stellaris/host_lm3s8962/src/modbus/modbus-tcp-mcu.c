...
static ssize_t _modbus_tcp_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    return send(ctx->s, (const char *)req, (size_t)req_length, MSG_NOSIGNAL);
}
...
static ssize_t _modbus_tcp_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length) {
    return recv(ctx->s, (char *)rsp, (size_t)rsp_length, 0);
}
                                              ...
                                              const uint8_t *rsp, int rsp_length)
{
    ...
    if (req[0] != rsp[0] || req[1] != rsp[1]) {
        if (ctx->debug) {
            printf("Invalid transaction ID received 0x%X (not 0x%X)\n",
    ...
    if (rsp[2] != 0x0 && rsp[3] != 0x0) {
        if (ctx->debug) {
            printf("Invalid protocol ID received 0x%X (not 0x0)\n",
                    (rsp[2] << 8) + rsp[3]);
        }
        errno = EMBBADDATA;
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    if (node == NULL) {
        ...
        if (ret_size == 0) {
            printf("The node string is empty\n");
            modbus_free(ctx);
            errno = EINVAL;
        ...
        if (ret_size >= dest_size) {
            printf("The node string has been truncated\n");
            modbus_free(ctx);
            errno = EINVAL;
    ...
    if (ret_size == 0) {
        printf("The service string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
    ...
    if (ret_size >= dest_size) {
        printf("The service string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
