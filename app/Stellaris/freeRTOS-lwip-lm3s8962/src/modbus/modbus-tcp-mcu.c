...
static size_t _modbus_tcp_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    return lwip_send(ctx->s, (const char *)req, (size_t)req_length, MSG_NOSIGNAL);
}
...
static size_t _modbus_tcp_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length) {
    return lwip_recv(ctx->s, (char *)rsp, (size_t)rsp_length, 0);
}
                                              ...
                                              const uint8_t *rsp, int rsp_length)
{
    ...
    if (req[0] != rsp[0] || req[1] != rsp[1]) {
        if (ctx->debug) {
            UARTprintf("Invalid transaction ID received 0x%X (not 0x%X)\n",
    ...
    if (rsp[2] != 0x0 && rsp[3] != 0x0) {
        if (ctx->debug) {
            UARTprintf("Invalid protocol ID received 0x%X (not 0x0)\n",
                    (rsp[2] << 8) + rsp[3]);
        }
        errno = EMBBADDATA;
...
static int _modbus_tcp_set_ipv4_options(int s)
{
    ...
    rc = lwip_setsockopt(s, IPPROTO_IP, IP_TOS,
                    (const void *)&option, sizeof(int));
    if (rc == -1) {
        return -1;
    }
...
static int _modbus_tcp_pi_connect(modbus_t *ctx)
{
    ...
    if (rc != 0) {
        if (ctx->debug) {
            UARTprintf("Error returned by getaddrinfo: %d\n", rc);
        }
        errno = ECONNREFUSED;
...
int modbus_tcp_listen(modbus_t *ctx, int nb_connection)
{
    ...
    new_s = lwip_socket(PF_INET, flags, IPPROTO_TCP);
    if (new_s == -1) {
        return -1;
    }
    ...
    addr.sin_port = lwip_htons((uint16_t)ctx_tcp->port);
    if (ctx_tcp->ip[0] == '0') {
...
int modbus_tcp_accept(modbus_t *ctx, int *s)
{
    ...
    if (ctx->debug) {
        UARTprintf("The client connection from %s is accepted\n",
...
int modbus_tcp_pi_accept(modbus_t *ctx, int *s)
{
    ...
    if (ctx->debug) {
        UARTprintf("The client connection is accepted.\n");
    }
...
static int _modbus_tcp_select(modbus_t *ctx, fd_set *rset, struct timeval *tv, int length_to_read)
{
    ...
    while ((s_rc = lwip_select(ctx->s+1, rset, NULL, NULL, tv)) == -1) {
        if (errno == EINTR) {
            if (ctx->debug) {
                UARTprintf("A non blocked signal was caught\n");
            }
...
static const modbus_backend_t _modbus_tcp_backend = {
...
static const modbus_backend_t _modbus_tcp_pi_backend = {
    _MODBUS_BACKEND_TYPE_TCP,
    _MODBUS_TCP_HEADER_LENGTH,
    _MODBUS_TCP_CHECKSUM_LENGTH,
    MODBUS_TCP_MAX_ADU_LENGTH,
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
    ...
    ctx->backend_data = (modbus_tcp_t *)platform_malloc(sizeof(modbus_tcp_t));
    ...
    if (ip != NULL) {
        ...
        if (ret_size == 0) {
            UARTprintf("The IP string is empty\n");
            modbus_free(ctx);
            errno = EINVAL;
        ...
        if (ret_size >= dest_size) {
            UARTprintf("The IP string has been truncated\n");
            modbus_free(ctx);
            errno = EINVAL;
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    ctx->backend_data = (modbus_tcp_pi_t *)platform_malloc(sizeof(modbus_tcp_pi_t));
    if (ctx->backend_data == NULL) {
        modbus_free(ctx);
        errno = ENOMEM;
    ...
    if (node == NULL) {
        ...
        if (ret_size == 0) {
            UARTprintf("The node string is empty\n");
            modbus_free(ctx);
            errno = EINVAL;
        ...
        if (ret_size >= dest_size) {
            UARTprintf("The node string has been truncated\n");
            modbus_free(ctx);
            errno = EINVAL;
    ...
    if (ret_size == 0) {
        UARTprintf("The service string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
    ...
    if (ret_size >= dest_size) {
        UARTprintf("The service string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
