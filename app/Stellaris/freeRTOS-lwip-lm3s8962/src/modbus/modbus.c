...
{
    if (ctx->debug) {
        UARTprintf("ERROR %s", modbus_strerror(errno));
        if (context != NULL) {
            UARTprintf(": %s\n", context);
        } else {
            UARTprintf("\n");
...
static void _sleep_response_timeout(modbus_t *ctx)
{
	...
	int msSleep = (ctx->response_timeout.tv_sec * 1000) + (ctx->response_timeout.tv_usec / 1000);
	vTaskDelay(msSleep);
	UARTprintf("delay %dms\r\n", msSleep);
}
...
int modbus_flush(modbus_t *ctx)
{
    ...
    if (rc != -1 && ctx->debug) {
        ...
        UARTprintf("Bytes flushed (%d)\n", rc);
    }
    return rc;
}

/* Computes the length of the expected response */
static int compute_response_length_from_request(modbus_t *ctx, uint8_t *req)
{
    int length;
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if (ctx->debug) {
        if (msg_type == MSG_INDICATION) {
            UARTprintf("Waiting for an indication...\n");
        } else {
            UARTprintf("Waiting for a confirmation...\n");
        }
    }
    ...
    while (length_to_read != 0) {
        rc = (int)ctx->backend->select(ctx, &rset, p_tv, length_to_read);
        if (rc == -1) {
        ...
        if (ctx->debug) {
            int i;
            for (i=0; i < rc; i++)
                UARTprintf("<%02X>", msg[msg_length + i]);
        }
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    ...
    if (nb > MODBUS_MAX_WRITE_BITS) {
        if (ctx->debug) {
            UARTprintf("ERROR Writing too many bits (%d > %d)\n",
                    nb, MODBUS_MAX_WRITE_BITS);
        }
        errno = EMBMDATA;
...
int modbus_report_slave_id(modbus_t *ctx, int max_dest, uint8_t *dest)
{
    ...
    if (rc > 0) {
        ...
        offset = (int)ctx->backend->header_length + 2;
...
int modbus_connect(modbus_t *ctx)
{
    ...
    return (int)ctx->backend->connect(ctx);
}
...
void modbus_close(modbus_t *ctx)
{
    ...
    const modbus_backend_t *backend = ctx->backend;
    backend->close(ctx);
}
    ...
    unsigned int start_input_registers, unsigned int nb_input_registers)
{
    ...
    mb_mapping = (modbus_mapping_t *)platform_malloc(sizeof(modbus_mapping_t));
    if (mb_mapping == NULL) {
        return NULL;
    }
    ...
    if (nb_bits == 0) {
        ...
        mb_mapping->tab_bits =
            (uint8_t *) platform_malloc(nb_bits * sizeof(uint8_t));
        if (mb_mapping->tab_bits == NULL) {
            platform_free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_input_bits == 0) {
        ...
        mb_mapping->tab_input_bits =
            (uint8_t *) platform_malloc(nb_input_bits * sizeof(uint8_t));
        if (mb_mapping->tab_input_bits == NULL) {
            platform_free(mb_mapping->tab_bits);
            platform_free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_registers == 0) {
        ...
        mb_mapping->tab_registers =
            (uint16_t *) platform_malloc(nb_registers * sizeof(uint16_t));
        if (mb_mapping->tab_registers == NULL) {
            platform_free(mb_mapping->tab_input_bits);
            platform_free(mb_mapping->tab_bits);
            platform_free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_input_registers == 0) {
        ...
        mb_mapping->tab_input_registers =
            (uint16_t *) platform_malloc(nb_input_registers * sizeof(uint16_t));
        if (mb_mapping->tab_input_registers == NULL) {
            platform_free(mb_mapping->tab_registers);
            platform_free(mb_mapping->tab_input_bits);
            platform_free(mb_mapping->tab_bits);
            platform_free(mb_mapping);
            return NULL;
...
modbus_mapping_t* modbus_mapping_new_start_address2(
    unsigned int start_bits, unsigned int nb_bits,
    unsigned int start_input_bits, unsigned int nb_input_bits,
    unsigned int start_registers, unsigned int nb_registers,
    unsigned int start_input_registers, unsigned int nb_input_registers,
	uint8_t *table_bits, uint8_t *table_input_bits,
	uint16_t *table_registers, uint16_t *table_input_register)
{
    modbus_mapping_t *mb_mapping;
    ...
    mb_mapping->nb_bits = nb_bits;
    mb_mapping->start_bits = start_bits;
    mb_mapping->tab_bits = table_bits;

    /* 1X */
    mb_mapping->nb_input_bits = nb_input_bits;
    mb_mapping->start_input_bits = start_input_bits;
    mb_mapping->tab_input_bits = table_input_bits;

    /* 4X */
    mb_mapping->nb_registers = nb_registers;
    mb_mapping->start_registers = start_registers;
    mb_mapping->tab_registers = table_registers;

    /* 3X */
    mb_mapping->nb_input_registers = nb_input_registers;
    mb_mapping->start_input_registers = start_input_registers;
    mb_mapping->tab_input_registers = table_input_register;
                                     ...
                                     int nb_registers, int nb_input_registers)
{
    return modbus_mapping_new_start_address(
    		0,					// start address of bits
    		nb_bits,			// number of bits
			0,					// start address of inputs bits
			nb_input_bits,		// number of inputs bits
			0,					// start address of holding registers
			nb_registers,		// number of holding registers
			0,					// start of input registers
			nb_input_registers);// number of input register
}

modbus_mapping_t* modbus_mapping_new2(int nb_bits, int nb_input_bits,
                                     int nb_registers, int nb_input_registers,
									 uint8_t *table_bits, uint8_t *table_input_bits,
									 uint16_t *table_registers, uint16_t *table_input_register)
{
    return modbus_mapping_new_start_address2(
    		0,					// start address of bits
    		nb_bits,			// number of bits
			0,					// start address of inputs bits
			nb_input_bits,		// number of inputs bits
...
void modbus_mapping_free(modbus_mapping_t *mb_mapping)
{
    ...
    platform_free(mb_mapping->tab_input_registers);
    platform_free(mb_mapping->tab_registers);
    platform_free(mb_mapping->tab_input_bits);
    platform_free(mb_mapping->tab_bits);
    platform_free(mb_mapping);
